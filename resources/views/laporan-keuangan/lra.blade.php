...
<body>
    ...
    <div style="text-align: center; margin-bottom: 20px;">
        ...
        <p class="subtitle">Untuk Tahun yang Berakhir pada 31 Desember {{ $tahun }}</p>
    </div>
    ...
    <table>
        ...
        <tbody>
            <tr>
                <td colspan="5" class="font-bold bg-light">PENDAPATAN</td>
            </tr>
            @php $totAnggP = 0; $totRealP = 0; @endphp
            @foreach($data['pendapatan'] as $p)
            @php 
                $totAnggP += $p->anggaran; $totRealP += $p->realisasi; 
                $persen = $p->anggaran > 0 ? ($p->realisasi / $p->anggaran) * 100 : 0;
            @endphp
            <tr>
                <td class="text-center">{{ $p->kode_akun }}</td>
                <td>{{ $p->nama_akun }}</td>
                <td class="text-right">{{ number_format($p->anggaran, 2, ',', '.') }}</td>
                <td class="text-right">{{ number_format($p->realisasi, 2, ',', '.') }}</td>
            ...
            <tr class="font-bold bg-light">
                ...
                <td class="text-right">{{ number_format($totAnggP, 2, ',', '.') }}</td>
                <td class="text-right">{{ number_format($totRealP, 2, ',', '.') }}</td>
                <td class="text-center">{{ $totAnggP > 0 ? number_format(($totRealP / $totAnggP) * 100, 2, ',', '.') : 0 }} %</td>
            </tr>

            <tr>
                <td colspan="5" class="font-bold bg-light">BELANJA / BEBAN</td>
            </tr>
            @php $totAnggB = 0; $totRealB = 0; @endphp
            @foreach($data['belanja'] as $b)
            @php 
                $totAnggB += $b->anggaran; $totRealB += $b->realisasi; 
                $persen = $b->anggaran > 0 ? ($b->realisasi / $b->anggaran) * 100 : 0;
            @endphp
            <tr>
                <td class="text-center">{{ $b->kode_akun }}</td>
                <td>{{ $b->nama_akun }}</td>
                <td class="text-right">{{ number_format($b->anggaran, 2, ',', '.') }}</td>
                <td class="text-right">{{ number_format($b->realisasi, 2, ',', '.') }}</td>
                <td class="text-center">{{ number_format($persen, 2, ',', '.') }} %</td>
            </tr>
            @endforeach
            <tr class="font-bold bg-light">
                <td colspan="2" class="text-center">TOTAL BELANJA</td>
                <td class="text-right">{{ number_format($totAnggB, 2, ',', '.') }}</td>
                <td class="text-right">{{ number_format($totRealB, 2, ',', '.') }}</td>
                <td class="text-center">{{ $totAnggB > 0 ? number_format(($totRealB / $totAnggB) * 100, 2, ',', '.') : 0 }} %</td>
            </tr>

            <tr class="font-bold">
                <td colspan="2" class="text-center">SURPLUS / (DEFISIT)</td>
                <td class="text-right">{{ number_format($totAnggP - $totAnggB, 2, ',', '.') }}</td>
                <td class="text-right">{{ number_format($totRealP - $totRealB, 2, ',', '.') }}</td>
                <td></td>
            </tr>
        </tbody>
    </table>
