...
<body>
    ...
    <table>
        ...
        <tbody>
            <tr>
                <td colspan="3" class="font-bold bg-light">PENDAPATAN LO</td>
            </tr>
            @foreach($data['pendapatan_lo'] as $p)
            <tr>
                <td class="text-center">{{ $p->kode_akun }}</td>
                <td>{{ $p->nama_akun }}</td>
                <td class="text-right">0,00</td> <!-- Mocked for now -->
            </tr>
            @endforeach

            <tr>
                <td colspan="3" class="font-bold bg-light">BEBAN</td>
            </tr>
            @foreach($data['beban'] as $b)
            <tr>
